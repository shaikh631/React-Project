...
function Hero() {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`absolute top-20 right-10 w-72 h-72 rounded-full filter blur-3xl opacity-20 animate-pulse ${
          isDarkMode ? 'bg-blue-700' : 'bg-blue-200 mix-blend-multiply'
        }`}
        style={{ transform: `translateY(${scrollY * 0.5}px)` }}
      ></div>
      <div 
        className={`absolute -bottom-8 left-10 w-72 h-72 rounded-full filter blur-3xl opacity-20 animate-pulse ${
          isDarkMode ? 'bg-cyan-700' : 'bg-cyan-200 mix-blend-multiply'
        }`}
        style={{ transform: `translateY(${scrollY * -0.3}px)` }}
      ></div>
      
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center min-h-[550px]">
          ...
          >
            {/* Badge */}
            <div className={`inline-flex items-center gap-2 px-4 py-2 border rounded-full w-fit shadow-sm hover:shadow-md transition-all duration-300 ${
              isDarkMode ? 'border-blue-800 bg-slate-900/80 shadow-slate-950/30' : 'border-blue-200 bg-white'
            }`}>
              <svg className="w-5 h-5 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/>
              </svg>
              <span className={`text-sm font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>Trusted Insurance Provider</span>
            </div>

            {/* Heading */}
            <div>
              <h1 className={`text-5xl sm:text-5xl lg:text-6xl font-black leading-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                Protect What <br />
            ...
            {/* Description */}
            <p className={`text-lg leading-relaxed max-w-xl ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              <button className="group px-8 py-3 bg-linear-to-r from-blue-600 to-blue-700 text-white font-bold rounded-lg shadow-lg hover:shadow-2xl hover:shadow-blue-500/40 transition-all duration-300 hover:scale-105 active:scale-95">
              ...
              <button className={`group px-8 py-3 border-2 border-blue-600 font-bold rounded-lg transition-all duration-300 ${
                isDarkMode ? 'bg-slate-900 text-blue-300 hover:bg-slate-800' : 'bg-white text-blue-600 hover:bg-blue-50'
              }`}>
                Learn More
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-6 pt-8">
              <div className="flex flex-col items-start">
                <div className="flex items-center gap-2">
                  <svg className="w-6 h-6 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17 20h5v-2a3 3 0 00-5.856-1.487M16 7a4 4 0 11-8 0 4 4 0 018 0zM9 20H4v-2a6 6 0 0112 0v2H9z"/>
                  </svg>
                  <p className="text-3xl font-bold text-blue-600">500K+</p>
                </div>
                <p className={`font-medium text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-700'}`}>Customers</p>
              </div>
              ...
              <div className="flex flex-col items-start">
                ...
                <p className={`font-medium text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-700'}`}>Approval</p>
              </div>
              ...
              <div className="flex flex-col items-start">
                ...
                <p className={`font-medium text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-700'}`}>Support</p>
              </div>
            </div>
          </div>

          {/* Right Visual Element with 3D Parallax */}
          <div className="hidden md:flex justify-center items-center perspective">
            ...
            >
              ...
              <div className={`absolute top-10 left-10 w-16 h-16 rounded-full shadow-lg flex items-center justify-center border-2 hover:shadow-xl transition-all ${
                isDarkMode ? 'border-blue-900 bg-slate-800' : 'border-blue-100 bg-white'
              }`}>
                <svg className="w-8 h-8 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.92 6.01C18.57 2.18 15.38 0 11.99 0c-3.45 0-6.5 2.2-7.73 5.3C2.5 5.7 0 8.2 0 11c0 3.32 2.68 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.99z"/>
                </svg>
              </div>

              <div className={`absolute bottom-10 left-20 w-20 h-20 rounded-full shadow-lg flex items-center justify-center border-2 hover:shadow-xl transition-all ${
                isDarkMode ? 'border-blue-900 bg-slate-800' : 'border-blue-100 bg-white'
              }`}>
                <svg className="w-10 h-10 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M3 12l2-4m0 0l7-4 5 10L15 8m6 0l2 4m-2-4v10c0 1-1 2-2 2H5c-1 0-2-1-2-2V8m0 0L3 4m0 0h10m0 0l4 4"/>
                </svg>
              </div>

              <div className={`absolute top-32 right-10 w-16 h-16 rounded-full shadow-lg flex items-center justify-center border-2 hover:shadow-xl transition-all ${
                isDarkMode ? 'border-blue-900 bg-slate-800' : 'border-blue-100 bg-white'
              }`}>
              ...
              <div className={`absolute right-0 bottom-5 w-20 h-20 rounded-full shadow-lg flex items-center justify-center border-2 hover:shadow-xl transition-all ${
                isDarkMode ? 'border-blue-900 bg-slate-800' : 'border-blue-100 bg-white'
              }`}>
                <svg className="w-10 h-10 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
              ...
              {/* Main Shield Card */}
              <div 
                className={`relative z-20 rounded-2xl shadow-2xl p-8 w-72 h-80 flex items-center justify-center border-2 hover:border-blue-300 transition-all duration-300 ${
                  isDarkMode ? 'border-blue-900 bg-slate-800 shadow-slate-950/50' : 'border-blue-100 bg-white'
                }`}
              >
                <div className="relative">
                  {/* Large Shield */}
                  <svg className="w-56 h-56 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 20c-4.42-1.29-7-5.6-7-10V6.7L12 3.4l7 3.3V11c0 4.4-2.58 8.71-7 10z"/>
                  </svg>
                  
                  {/* Family Icon Overlay */}
                  <svg className="absolute top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-24 h-24 text-blue-500 opacity-40" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col justify-start lg:order-1 space-y-4 py-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}>Guaranteed Coverage</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}>Tax Efficient</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}>24/7 Support</span>
              </div>
            </div>

            <button className="w-fit px-5 py-2 bg-linear-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-lg shadow-md hover:shadow-lg hover:shadow-blue-500/30 transition-all duration-300 hover:scale-105 active:scale-95 text-xs mt-2">
              Learn More
            </button>
          </div>
        </div>
